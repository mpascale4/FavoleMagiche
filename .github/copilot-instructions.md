...
| `:rundev` | Verifica se la porta è occupata (chiede se riavviare); altrimenti lancia `npm run dev` in background/detached con log su `logs/dev-server.log`. |
| `:dev` | Lancia subito `npm run dev` (async, attaccato alla sessione corrente, si ferma a fine sessione), senza chiedere conferma. |
...
- (Nessun learning specifico registrato ancora per questo repository: aggiungere qui le lezioni apprese durante le sessioni, seguendo lo stesso formato usato in Tabellandia/GiocoDeiMostri.)
